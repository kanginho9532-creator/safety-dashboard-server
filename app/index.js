...
app.get('/api/companies', async (req, res) => {
  try {
    ...
    const companies = buildCompanies(rawSites, companyPages);
...
app.post('/api/company-card', async (req, res) => {
  try {
    const { companyName, responsible = '', people = [] } = req.body || {};
    if (!companyName) return res.status(400).json({ ok: false, error: 'companyName is required' });
    const result = await updateCompanyPages(companyName, { companyName, responsible, people });
