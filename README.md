...
- **PC(가로폭 1025px 이상)**: 기존처럼 좌측 고정 사이드바와 넓은 2열/5열 레이아웃으로 표시됩니다.
- **모바일·태블릿(가로폭 1024px 이하)**: 상단에 **모바일 전용 헤더**가 나타나고, 좌측 메뉴는 **햄버거 버튼(☰ 메뉴)** 으로 여닫는 전용 화면으로 동작합니다.
- 모바일에서 표/캘린더는 내용을 유지한 채 **가로 스크롤** 가능하도록 처리되어, 데이터가 잘리지 않고 터치로 확인할 수 있습니다.
- 모바일에서 메뉴 항목을 누르면 메뉴가 자동으로 닫혀 본문 화면에 바로 집중할 수 있습니다.
- 상단의 **동기화** 버튼으로 모바일에서도 즉시 Notion 재동기화할 수 있습니다.
